...
现在你可以通过配置好的 AI 客户端（比如 Claude Code、Cursor）输入需求，例如：

> “我想做一个 60 秒的中文科普视频，主题是‘为什么天空是蓝色的’，风格轻松有趣，适合发抖音。”
...
渲染完成后，视频文件通常会保存在项目的 `output/` 或 `projects/` 目录下。你可以直接在文件夹里找到 MP4 文件播放。
...
1. **第一次用，先跑 demo**。不要一上来就填一堆 API Key，先看看默认示例能不能跑通。
...
> **让一个完全不懂剪辑的普通人，也能通过和 AI 对话，生产出结构完整、风格统一、带有中文字幕和配音的视频。**
...
如果你已经按照上面的步骤安装好了，不妨现在就试试：

> “帮我做一个 60 秒的中文短视频，主题是……”
