...
2. **中文配置向导**：运行 `python3 scripts/config_wizard.py` 即可交互式配置 LLM、图像/视频/TTS/音乐提供商，支持 DeepSeek、通义千问、智谱、Moonshot、百川、可灵、海螺、豆包、通义听悟等国内主流模型，以及 AutoDL 模型广场上的对话模型（DeepSeek-V4-Pro、GLM-5.2、Qwen3.7-Max 等）、生图模型（gpt-image-2 等）和生视频模型（doubao-seedance-2-0 等）。
