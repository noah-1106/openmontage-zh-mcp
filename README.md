...
<p align="center"><strong>基于 OpenMontage 的中文界面 + MCP Server + 国内大模型友好适配版</strong></p>

<p align="center">
  <a href="#mcp-服务">MCP 服务</a> &nbsp;·&nbsp;
  <a href="#我们的改动与更新">改动与更新</a> &nbsp;·&nbsp;
  <a href="#快速开始">快速开始</a> &nbsp;·&nbsp;
  <a href="#原项目介绍">原项目介绍</a>
</p>
...
本仓库提供了 OpenMontage 的 **MCP Server**，外部智能体（Claude Code、Cursor、Copilot 等）可通过 stdio 协议直接调用其工具与流水线能力，无需常驻端口。
...
3. **国内大模型与模型广场支持**：在 OpenMontage 原版已支持的 Anthropic、OpenAI、DeepSeek、通义千问、智谱、Moonshot、百川、可灵、海螺、豆包、通义听悟等模型之外，新增接入 [AutoDL 模型广场](https://autodl.art/large-model/market)。一次配置 `AUTODL_API_KEY` 即可使用对话模型（DeepSeek-V4-Pro、GLM-5.2、Qwen3.7-Max 等）、生图模型（gpt-image-2 等）和生视频模型（doubao-seedance-2-0 等），更方便国内用户一站式调用。
...
# ⬇️ 下面是原版 OpenMontage 介绍 ⬇️

**[English README](README_original.md)** · **[中文 README](README_zh-CN.md)**
